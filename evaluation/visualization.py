...
def plot_error_overlap(
    overlap_tables: dict[str, pd.DataFrame],
    figsize: tuple[int, int] | None = None,
    return_fig: bool = False,
) -> dict[str, pd.DataFrame] | tuple[dict[str, pd.DataFrame], plt.Figure]:
    """Heatmap panel of per-category error-overlap (Jaccard) matrices.

    ``overlap_tables`` maps a category label to a square models×models DataFrame of
    error-set Jaccard values (as returned by
    ``evaluation.complementarity.error_overlap_matrix``). One heatmap per category; high
    values (near 1.0) mean the members make the same mistakes — no diversity for fusion
    to exploit. Returns the input dict, plus the figure when ``return_fig=True``.
    """
    labels = list(overlap_tables)
    n = len(labels)
    figsize = figsize or (4 * n, 3.6)
    fig, axes = plt.subplots(1, n, figsize=figsize)
    if n == 1:
        axes = [axes]

    for ax, label in zip(axes, labels):
        m = overlap_tables[label]
        sns.heatmap(
            m,
            annot=True,
            fmt=".2f",
    ...
    fig.suptitle("Members make correlated errors → no diversity for fusion to exploit")
    fig.tight_layout()
    if return_fig:
        return overlap_tables, fig
