"""Fine-tuning entry point for BERT DLP classifier.

Usage:
    python -m models.bert.train [--epochs 5] [--batch_size 16] [--lr 2e-5]
...
def fit(
    ...
    """Fine-tune BERT on ``train_df``, selecting the checkpoint on ``val_df``.
    ...
    ``(ckpt_path, best_val_macro_f1)``. The CLI ``train`` and the k-fold orchestrator
    share this function so their training regimes are identical.
    """
