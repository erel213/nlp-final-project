...
ensemble obtains the highest point estimate, but its advantage over
RoBERTa is less than one macro-F1 percentage point. The main result is
therefore not a large performance improvement. Instead, it shows that a
strong contextual transformer already captures most of the predictive
...
uniform across categories. RoBERTa receives approximately 0.8 of the
weight for both benign and general PII predictions, showing that these
decisions are dominated by contextual representations. For financial
information, however, the weight is distributed across RoBERTa
(approximately 0.52), the rule-based detector (0.29), and the BiLSTM
(0.19). This is consistent with the nature of financial identifiers:
many have recognizable surface structure, so regular expressions and
character-level features can provide signals that complement contextual
modeling. For confidential information, RoBERTa remains dominant, while
BERT and the rule-based detector retain smaller contributions.
